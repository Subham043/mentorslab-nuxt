<template>
    ...
    <section class="content">
        <div class="box-header d-flex justify-content-between align-items-center box-header-user">
            <UserCrumbComponent main-page="Assessment" current-page="Paid" />
        </div>
...
import NoUserDataComponent from '~/components/NoUserDataComponent.vue';
  export default {
    ...
    async getTableData(page=0) {
        this.loading=true
        try {
            const response = await this.$privateApi.get('/assessment-user/paginate-paid?skip='+page); // eslint-disable-line
